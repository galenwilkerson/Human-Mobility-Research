...
df.time.unclassed <- data.frame(trips, unclass(as.POSIXlt(trips$timestamp.arrival)))

plot(df.time.unclassed$hour, df.time.unclassed$duration.min)

h1 <- hist(df.time.unclassed$hour, breaks = 24, plot=F)
plot(h1, border = "black", xlab="Hour", ylab="Number of Arrivals", main=paste("Arrivals:", breaks.interval, "averaged over entire time period"))
...
h1 <- hist(df.time.unclassed$hour, breaks = 24, plot=F)
plot(h1, border = colors[index], xlab="Hour", ylab="Number of Arrivals", main=paste("Arrivals:", breaks.interval, "averaged over entire time period"))
...
h1 <- hist(df.time.unclassed$hour, breaks = 24, plot=F)
lines(h1$count, type = "s", col = colors[index], xlab="Hour", ylab="Number of Arrivals",  main=paste("Arrivals:", breaks.interval))
