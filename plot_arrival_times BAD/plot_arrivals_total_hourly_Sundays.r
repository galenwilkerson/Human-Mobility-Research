...
# use only weekdays
df.time.unclassed <- subset(df.time.unclassed, wday == 0)


h1 <- hist(df.time.unclassed$hour, breaks = 24, plot=F)
plot(h1, border = "black", xlab="Hour", ylab="Number of Arrivals", main=paste("Sunday Arrivals:", breaks.interval, "averaged over entire time period"))
...
h1 <- hist(df.time.unclassed$hour, breaks = 24, plot=F)
plot(h1, border = colors[index], xlab="Hour", ylab="Number of Arrivals", main=paste("Sunday Arrivals:", breaks.interval, "averaged over entire time period"))
...
for (index in 3:length(trip.modes)) {
