...
legend.labels <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")
...
write.csv(results, "day_fit_results.csv")
...
plot1 <-ggplot(NULL) +
    ## # draw fit line
  geom_line(data = best.fit.df, aes(x = log10.trip.length, y = log10.P.X, colour = cat.val, group=cat.val), linetype = 2, size = .5, alpha = .7) +
