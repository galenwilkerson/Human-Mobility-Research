...
# make nice plot of trip purpose
...
# REMOVE ALL BUT INTERESTING VALUES

trips.subset <- subset(trips.subset, s04 == cat.vals[1] | s04 == cat.vals[2] | s04 == cat.vals[4])

cat.vals <-  as.vector(unique(trips.subset[,this.cat]))
...
legend.labels <- c("yes, always", "not at all", "sometimes")
...
best.fit.df <- rbind(best.fit.df, data.frame(log10.trip.length, log10.P.X, cat.val, order))
    ...
    print("var:")
    print(var(trips.subset.cat$wegkm.k))
...
plot1 <-ggplot(NULL) +
 ...
 # the transparency values
  ...
  scale_colour_brewer(palette="Set1",
                      name="Motor vehicle availability",
                      breaks=cat.vals,
